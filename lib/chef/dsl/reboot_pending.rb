    ...
    module RebootPending
      ...
      def reboot_pending?
        ...
        if platform_family?("windows")
          ...
          registry_value_exists?('HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\PendingFileRenameOperations') ||
          ...
          (registry_key_exists?('HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\WindowsUpdate\Auto Update\RebootRequired') &&
                registry_get_values('HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\WindowsUpdate\Auto Update\RebootRequired').select{|v| v[:data] == 1 }.any?) ||
          ...
          (registry_key_exists?('HKLM\SOFTWARE\Microsoft\Updates\UpdateExeVolatile') &&
                registry_get_values('HKLM\SOFTWARE\Microsoft\Updates\UpdateExeVolatile').select { |v| v[:name] == "Flags" }[0] &&
                [1,2,3].include?(registry_get_values('HKLM\SOFTWARE\Microsoft\Updates\UpdateExeVolatile').select { |v| v[:name] == "Flags" }[0][:data])) ||

          # windows_reboot LWRP from windows cookbook
          ...
          Chef::Log.error("reboot_pending? is not supported on platform '#{node[:platform_family]}'.")
